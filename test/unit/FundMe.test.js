...
if (!developmentChains.includes(network.name)) {
    ...
    describe('FundMe', async () => {
        ...
        describe('withdraw', async () => {
            ...
            it('should allow us to cheap_withdraw with multiple funders', async () => {
                const accounts = await ethers.getSigners();
                for (index = 1; index < accounts.length; index++) {
                    const fundMeConnectedContract = await fundMe.connect(accounts[index]);
                    await fundMeConnectedContract.fund({ value: sendValue });
            ...
            it('should only allow owner to cheap_withdraw', async () => {
                const accounts = await ethers.getSigners();
                const fundMeConnectedContract = await fundMe.connect(accounts[1]);
                // await fundMeConnectedContract.withdraw();
                await expect(fundMeConnectedContract.cheap_withdraw()).to.be.revertedWith('FundMe__NotOwner');
            });
        });
    });
}